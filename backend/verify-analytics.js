        ...
        .forEach((courseCode) => {
          ...
          course.offerings.forEach((o) => {
            ...
            const uniqueStudents = new Set(
              o.enrollments.map((e) => e.student?.id)
            ).size;

            console.log(
              `       Section ${sectionName}: ${enrollmentCount} enrollments (${uniqueStudents} unique students)`
            );

            if (enrollmentCount > 0 && enrollmentCount <= 3) {
              o.enrollments.forEach((e) => {
                console.log(
                  `         - ${e.student?.usn} (Section ${e.student?.sections?.section_name})`
                );
              });
            }
          });
