...
async function fixCS301Offerings() {
  try {
    ...
    if (wrongOffering) {
      console.log(
        `⚠️  Found Section B offering in Semester 1: ${wrongOffering.id}`
      );
    ...
    const hasCOffering = currentOfferings.some(
      (o) => o.sections?.section_name === "C"
    );
    ...
    finalOfferings.forEach((o) => {
      console.log(
        `  ✓ Section ${o.sections?.section_name || "None"}: Teacher ${
