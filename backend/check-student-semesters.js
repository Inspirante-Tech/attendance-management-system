  ...
  try {
    ...
    if (cs301) {
      ...
      cs301.courseOfferings.forEach((offering) => {
        ...
        console.log(
          `  Academic Year: ${offering.academic_years?.year_name || "Unknown"}`
        );
        ...
        if (offering.enrollments.length > 0) {
          offering.enrollments.slice(0, 3).forEach((e) => {
            console.log(
              `    - ${e.student?.usn} (Sem ${e.student?.semester}, Section ${e.student?.sections?.section_name})`
            );
          });
