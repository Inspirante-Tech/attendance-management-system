...
async function createNMITcourseOfferings() {
  try {
    ...
    console.log(
      `Found ${nmamitCourses.length} courses in NMAMIT CS department\n`
    );
    ...
    console.log(
      `Found ${nmamitOfferings.length} NMAMIT offerings for Semester 5\n`
    );
    ...
    for (const offering of nmamitOfferings) {
      ...
      const nmitSection = nmitSections.find(
      ...
      if (existingOffering) {
        console.log(
          `  ✓ Offering already exists: ${offering.course.code} - Section ${sectionName}`
        );
        continue;
      }
      ...
      console.log(
        `  ✅ Created offering: ${offering.course.code} - Section ${sectionName}`
      );
      createdCount++;
    }
