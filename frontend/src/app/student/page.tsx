...
import { StudentInfo } from '@/types/student'
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      try {
        if (!studentId) { // you’ll get this from auth or route params
          const data  = await studentApi.getStudentProfile(studentId);
          setStudentData(data);
        }
      } catch (err) {
    ...
    const [marksData] = useState(mockStudentData.marksData)
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-2 sm:p-4 lg:p-6">
        <div className="max-w-7xl mx-auto space-y-3 sm:space-y-4 lg:space-y-6">        {/* Header */}
          ...
          <Card className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
            <CardHeader className="pb-3 sm:pb-4 lg:pb-6">
              <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto sm:mx-0">                {studentData ? (
                    <Image
                      src={studentData ? studentData.photo_url : 'test'} 
                      alt={studentData ? studentData.name : 'test'}
                      width={56}
                      height={56}
                ...
                <div className="flex-1 text-center sm:text-left">
                  <CardTitle className="text-white text-lg sm:text-xl lg:text-2xl">Welcome back, {studentData ? studentData.name : 'test'}</CardTitle>
                  <CardDescription className="text-blue-100 text-sm sm:text-base">
                    {studentData ? studentData.usn : 'test'} • Semester {studentData ? studentData.semester : 'test'} • {studentData ? studentData.department : 'test'}
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                <div className="flex items-center justify-center sm:justify-start space-x-2">
                  ...
                  <span className="text-xs sm:text-sm truncate">{studentData ? studentData.college_name : 'test'}</span>
                </div>
                <div className="flex items-center justify-center sm:justify-start space-x-2">
                  <Phone className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span className="text-xs sm:text-sm">{studentData ? studentData.phone : 'test'}</span>
                </div>
                <div className="flex items-center justify-center sm:justify-start space-x-2 sm:col-span-2 lg:col-span-1">
                  <Mail className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span className="text-xs sm:text-sm truncate">{studentData ? studentData.email : 'test'}</span>
                </div>
              </div>
            </CardContent>
          ...
          <div className="flex space-x-0.5 sm:space-x-1 bg-gray-100 p-0.5 sm:p-1 rounded-lg overflow-x-auto scrollbar-hide">
            ...
            <AttendanceStats studentId={studentData ? studentData.user_id : ''} compact={false} />
          )}        {activeTab === 'marks' && (
                      ...
                      <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">                      <table className="min-w-full divide-y divide-gray-300">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm font-medium text-gray-700">Theory</th>                            <th className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm font-medium text-gray-700">Record</th>
                          ...
                          <tbody className="divide-y divide-gray-200 bg-white">
                            {marksData.map((course, index) => (
                              <tr key={index} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="hidden lg:table-cell px-2 sm:px-3 py-2 sm:py-3">
                                ...
                                </td><td className="px-2 sm:px-3 py-2 sm:py-3 text-center">
                                  <span className={`inline-block px-1 py-0.5 text-xs rounded ${
                                    ...
                                    {course.course_type === 'core' ? 'Core' : 
                                    course.course_type === 'department_elective' ? 'Dept' : 'Open'}
                                  </span>
                                </td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_theory_component ? (
                                    <span className="font-medium">{course.theory_marks?.mse1_marks || '-'}</span>
                                  ) : (
                                ...
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_theory_component ? (
                                    <span className="font-medium">{course.theory_marks?.mse2_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_theory_component ? (
                                    <span className="font-medium">{course.theory_marks?.task1_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_theory_component ? (
                                    <span className="font-medium">{course.theory_marks?.task2_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_theory_component ? (
                                    <span className="font-medium">{course.theory_marks?.task3_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
                                ...
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_lab_component ? (
                                    <span className="font-medium">{course.lab_marks?.record_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
                                <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_lab_component ? (
                                    <span className="font-medium">{course.lab_marks?.continuous_evaluation_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>                              <td className="px-2 sm:px-3 py-2 sm:py-3 text-center text-xs sm:text-sm">
                                  {course.has_lab_component ? (
                                    <span className="font-medium">{course.lab_marks?.lab_mse_marks || '-'}</span>
                                  ) : (
                                    <span className="text-gray-400">N/A</span>
                                  )}
                                </td>
